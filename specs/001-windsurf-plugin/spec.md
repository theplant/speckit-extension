...
4. **Given** a spec without a `maturity.json` file, **When** the developer clicks "Copy for Test", **Then** a modal dialog appears with title "Initialize maturity.json", explaining that the file is recommended for test tracking, with two buttons: "Copy with AI Instructions" (copies context including maturity.json initialization instructions) and "Copy without Initialization" (copies context without initialization instructions)
...
7. **Given** no `maturity.json` file exists for the spec, **When** the developer clicks "Run Test" on any item (test, scenario, or user story), **Then** a modal dialog appears with title "Initialize maturity.json", explaining that the file is required for test tracking, with two buttons: "Copy AI Instructions" (copies initialization instructions to clipboard and shows confirmation) and "Cancel"
